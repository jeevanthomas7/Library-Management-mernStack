      ...
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4 mb-6">
        <div>
          <h1 className="text-3xl font-extrabold text-indigo-900 flex items-center gap-3">
            <span className="text-2xl">📚</span> Book Store
          </h1>
          <p className="text-sm text-slate-500 mt-1">
            Browse, search and Display books.
          </p>
        </div>
      ...
      {loading ? (
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">
          {Array.from({ length: 6 }).map((_, i) => (
            <div
              key={i}
              className="bg-white rounded-2xl shadow p-4 animate-pulse h-56"
            />
          ))}
        </div>
      ) : error ? (
            ...
            >
              ...
              <div className="p-4 flex-1 flex flex-col">
                ...
                <div className="mt-auto">
                  ...
                  <button className="w-full px-3 py-2 mt-3 bg-blue-400 text-slate-800 rounded-lg shadow hover:bg-blue-500 transition">
                    Show TO Display
                  </button>
                </div>
              </div>
